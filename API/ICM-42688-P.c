...
void ICM42688_INIT(I2C_HandleTypeDef *hi2cx)
{
    ...
    uint8_t Data;
        ...
        Data = 0xF;
        HAL_I2C_Mem_Write(hi2cx, ICM42688_ADDRESS, PWR_MGMT0, 1, &Data, 1, i2c_timeout);
        ...
        Data = 0x3;
        HAL_I2C_Mem_Write(hi2cx, ICM42688_ADDRESS, INT_CONFIG, 1, &Data, 1, i2c_timeout);

        // Set the configuration about when the INT1 is generate, INT_STATUS 0x2D
        // 0x100 to Active when DATA READY

        Data = 0x3;
        HAL_I2C_Mem_Write(hi2cx, ICM42688_ADDRESS, INT_STATUS, 1, &Data, 1, i2c_timeout);

        // Set accelerometer configuration to 0x50
        // ACCEL_FS_SEL = 0000011 -> 2g, ACCEL_ODR = 0110 -> 1kHz (default)

        Data = 0x66;
        HAL_I2C_Mem_Write(hi2cx, ICM42688_ADDRESS, ACCEL_CONFIG0, 1, &Data, 1, i2c_timeout);

        // Set accelerometer configuration to 0x4F
        // GYRO_FS_SEL = 011 -> 250dps, GYRO_ODR = 0110 -> 1kHz (default)

        Data = 0x66;
        HAL_I2C_Mem_Write(hi2cx, ICM42688_ADDRESS, GYRO_CONFIG0, 1, &Data, 1, i2c_timeout);
}

void ICM42688_Read_ACCEL(I2C_HandleTypeDef *hi2cx, ICM42688_t *DataStruct)
{
    uint8_t Rec_Data[6];

    // Read 6 BYTES of data starting from ACCEL_DATA_X1 register, 0x1F (HSB to LSB) and put on Rec_Data

    HAL_I2C_Mem_Read(hi2cx, ICM42688_ADDRESS, ACCEL_DATA_X1, 1, Rec_Data, 6, i2c_timeout);
...
void ICM42688_Read_GYRO(I2C_HandleTypeDef *hi2cx, ICM42688_t *DataStruct)
{
    uint8_t Rec_Data[6];

    // Read 6 BYTES of data starting from GYRO_XOUT_H register (HSB to LSB) and put on Rec_Data

    HAL_I2C_Mem_Read(hi2cx, ICM42688_ADDRESS, GYRO_DATA_X1, 1, Rec_Data, 6, i2c_timeout);
